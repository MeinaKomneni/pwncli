...
class Consts:
    ...
    @classmethod
    def _show_group(cls, name, grp, prefix=""):
        full = prefix + name
        ints = [(k, v) for k, v in grp.__dict__.items()
                if isinstance(v, int) and not k.startswith('_')]
        nested = [(k, v) for k, v in grp.__dict__.items()
                  if isinstance(v, type) and not k.startswith('_')]
        if ints:
            doc = (grp.__doc__ or "").strip()
            print(f"\n{'='*50}")
            print(f" {full}  —  {doc}")
            print(f"{'='*50}")
            for k, v in ints:
                if v < 0:
                    ...
                    print(f"  {k:<20s} = {v:#x} ({v})")
        for sub, subgrp in nested:
            cls._show_group(sub, subgrp, prefix=full + ".")
